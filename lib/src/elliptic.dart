...
abstract class Curve {
  ...
  AffinePoint add(AffinePoint p1, p2);
...
class JacobianPoint {
  BigInt X, Y, Z;
  JacobianPoint(this.X, this.Y, this.Z);
}
...
class PrivateKey {
  ...
  List<int> rand;
  BigInt D;
  PrivateKey(this.curve, this.rand) {
    var byteLen = (curve.BitSize + 7) ~/ 8;
    D = BigInt.parse(
        List<String>.generate(byteLen, (i) => rand[i].toRadixString(16)).join(),
        radix: 16);
  }
...
class PublicKey extends AffinePoint {
  Curve curve;
  PublicKey(this.curve, X, Y) : super(X, Y);

  PublicKey.fromPoint(this.curve, AffinePoint p) : super(p.X, p.Y);
...
class EllipticCurve implements Curve {
  ...
  BigInt Gx, Gy; // Gx&Gy are the x&y coordinate of the base point, respectively

  @override
  int BitSize; // BitSize is the size of the underlying secp256k1 field in bits.
  ...
  @override
  AffinePoint scalarMul(AffinePoint p, List<int> k) {
    var _p = JacobianPoint(BigInt.zero, BigInt.zero, BigInt.zero);
  ...
  @override
  AffinePoint scalarBaseMul(List<int> k) {
    return scalarMul(AffinePoint(Gx, Gy), k);
  }

  // addJacobian takes two points in Jacobian coordinates, (x1, y1, z1) and
  // (x2, y2, z2) and returns their sum, also in Jacobian form.
  JacobianPoint _addJacobian(BigInt x1, y1, z1, x2, y2, z2) {
  ...
  JacobianPoint _doubleJacobian(BigInt x, y, z) {
  ...
  AffinePoint _affineFromJacobian(BigInt x, y, z) {
    if (z.sign == 0) {
  ...
  PrivateKey generatePrivateKey() {
    ...
    return PrivateKey(this, rand);
  }

  @override
  PublicKey privateToPublicKey(PrivateKey priv) {
    return PublicKey.fromPoint(this, scalarBaseMul(priv.rand));
  }
...
// section 4.3.6 of ANSI X9.62.
  ...
  String publicKeyToHex(PublicKey pub) {
    ...
    ret += pub.X.toRadixString(16).padLeft(byteLen * 2);
    ret += pub.Y.toRadixString(16).padLeft(byteLen * 2);
...
// specified in section 4.3.6 of ANSI X9.62.
  ...
  String publicKeyToCompressedHex(PublicKey pub) {
    ...
    compressed += pub.X.toRadixString(16).padLeft(byteLen * 2);
